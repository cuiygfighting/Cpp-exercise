...
所有的函数都是平行的(包括main函数),在定义函数时是独立的，函数不能**嵌套定义**，既不能在定义一个函数的过程中又定义另一个函数(即函数1和函数2的声明都写在外面，但在函数1中定义函数2)，也不能把函数的定义部分写在主函数中。
...
定义：const int \**p=&a;
不允许通过指针变量改变它所指向的变量的值，即不能*\*p=2;	但可以改变指针变量的值，如可以p=&b;	变量仍可以改变，如a=2;
...
{                      //声明结构体类型时不分配空间，而定义结构体变量时需要分配空间
    ...
    Date birtgday      //Date是另一个结构体类型名，一个结构体中可以包含另一个结构体
    Student *p;        //指向自身类型的指针
...
C++提供new和delete来代替malloc和free，new和delete是运算符，不是函数，执行效率高。
...
4. 然后可以用新的类型名去定义变量，NUM a，a就是一个有10个元素的数组名，a[1]就是数组的第一个元素
...
不可访问指的是在派生类中不能直接访问基类的私有成员，但可以通过调用派生类中可以调用的基类成员的函数来间接调用甚至修改私有数据成员。无论哪一种继承方式，在派生类中都不能访问基类的私有成员。
...
建立一个派生类对象时，派生类的构造函数先调用基类构造函数对继承来的数据成员进行初始化，再对其子对象进行初始化，再执行派生类构造函数本身的函数体。
